AI_THINK_INTERVAL = 0.25 --//AI思考间隔时间
--AI 状态 constants
...
--定义 AINormal 类
AINormal = {}
AINormal.__index = AINormal

--[[通过一些参数和单位创建一个AINormal类的实例  ]]
function AINormal:MakeInstance( actor, params )
  --构造一个实例  of the AINormal class
  local ai = {}
  setmetatable( ai, AINormal )
...
function AINormal:GlobalThink()
  --单位死了就停止AI 
  if sc.IsAlive(self.unit)==false then
   ---TerminateMove 终止当前的移动
    -- return nil
  ...
  return
  -- --执行正确阶段的思考方法
  -- Dynamic_Wrap(AINormal, self.stateThinks[ self.state ])( self )
  --等待时间后继续调用这个方法
  -- return AI_THINK_INTERVAL
end

--[[ Idle状态的方法]]
function AINormal:IdleThink()
...
--[[ 积极状态的方法 ]]
function AINormal:AggressiveThink()
  local now_pos=sc.GetActorLogicPos(self.unit)
...
function AINormal:SetState(state)
...
--[[ 返回阶段的]]
function AINormal:ReturningThink()
  local now_pos=sc.GetActorLogicPos(self.unit)
